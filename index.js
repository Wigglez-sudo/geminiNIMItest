// NVIDIA AI Desktop - Cloudflare Worker proxy (canonical source)
// -------------------------------------------------------------
// This is the ONE Worker file. Copy it to your local Wrangler project's
// src/index.js and deploy:
//
//   cd C:\Users\lukew\OneDrive\Desktop\Nvidaapp\nvidia-ai-proxy\nvidia-ai-proxy
//   copy /Y <this file> src\index.js
//   npx wrangler deploy
//
// Routes:
//   GET         /                     -> status JSON (name, version, routes)
//   GET  /health, /v1/health          -> quick health probe
//   GET/OPTIONS /v1/models            -> proxied to NVIDIA
//   POST/OPTIONS /v1/chat/completions -> proxied to NVIDIA (streaming passed through)
//   POST/OPTIONS /v1/web-search       -> Brave (default) or Tavily
//   GET/OPTIONS  /v1/build-models     -> scrapes build.nvidia.com catalog
//
// NVIDIA auth accepts either header:
//   Authorization: Bearer nvapi-xxxx
//   X-Nvidia-Api-Key: nvapi-xxxx
//
// Search keys can be passed per-request (X-Search-Api-Key) or stored as
// Worker secrets so they never touch the browser:
//   npx wrangler secret put BRAVE_SEARCH_API_KEY
//   npx wrangler secret put TAVILY_API_KEY

const WORKER_VERSION = "2025.06-consolidated";
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health" || url.pathname === "/v1/health") {
      return jsonResponse({ ok: true, status: "healthy", version: WORKER_VERSION, time: new Date().toISOString() }, 200, origin);
    }
    ...
    if (!allowedPaths.includes(url.pathname)) {
      return jsonResponse({ ok: false, error: "Route not allowed", path: url.pathname, allowed: [...allowedPaths, "/v1/web-search", "/v1/build-models", "/health"] }, 404, origin);
    }
